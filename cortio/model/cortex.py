...
class Cortex:
    """Model of audio filtering by the auditory system and cortex
    filter_settings: a FilterSettings instance specifying filter settings
    """
    def __init__(self, filter_settings):
        self.filter_settings = filter_settings

    def wav2cor(self, wav, fs):
        """Transform audio into cortical filter representation
        wav : audio waveform
        fs  : sample rate
    ...
    def wav2aud(self, wav, fs):
        """Transform audio into log-freq spectrogram
        wav : audio waveform
        fs  : sample rate
        """
    ...
    def aud2cor(self, aud):
        """Transform spectrogram into cortical transform represetnation
        aud: spectrogram data, as generated by wav2aud
        Output dims: [scale, rate, time, freq]
        """
        cor = self.filter(aud)
        #collapse +/- rates
    ...
    def filter(self, specgram):
        """Cortical rate-scale representation filter

        Implements the 2-D wavelet transform modeled after A1 cortex.
        Rates and scales in the output are determined by
